...
-- A sequence of elements are all in the σ-algebra
...
class SigmaAlgebra (F : Set (Set Ωt)) : Prop where
  F_empty : ∅ ∈ F
...
-- TODO: Mathlib doesn't foce their definition of a measure
-- requires that P ∘ A is summable, why?
-- https://leanprover-community.github.io/mathlib4_docs/Mathlib/MeasureTheory/Measure/MeasureSpaceDef.html#MeasureTheory.Measure
class ProbabilityMeasure {M : MeasureSpace} (P : Set M.Ωt -> ℝ) where
  normalization: P M.Ω = 1
  countable_add (A : Nat -> Set M.Ωt) (H1 : cpd A) (H2 : inF M.F A) :
    P (⋃ i, A i) = (∑' i, P (A i))
...
class SemiMeasure {M : MeasureSpace} (P : Set M.Ωt -> Real) where
  leq: P M.Ω ≤ 1
  leq_countable_add (A : Nat -> Set M.Ωt) (H1 : cpd A) (H2 : inF M.F A) :
    P (⋃ i, A i) ≤ (∑' i, P (A i))
